...
main(int argc, char* argv[])
{
    ...
    Ipv4StaticRoutingHelper multicast, multicast1;

    // 1) Configure a (static) multicast route on node n2 (multicastRouter)
    Ptr<Node> multicastRouter = c.Get(2); // The node in question
    Ptr<NetDevice> inputIf = n0.Get(2);   // The input NetDevice
    NetDeviceContainer outputDevices;     // A container of output NetDevices
    outputDevices.Add(n1.Get(0));

    Ptr<Node> multicastRouter1 = c.Get(4); // The node in question
    Ptr<NetDevice> inputIf1 = n1.Get(2);   // The input NetDevice
    NetDeviceContainer outputDevices1;     // A container of output NetDevices
    outputDevices1.Add(n2.Get(0));         // (we only need one NetDevice here)

    Ptr<Node> multicastRouter2 = c.Get(5); // The node in question
    Ptr<NetDevice> inputIf2 = n2.Get(1);   // The input NetDevice
    NetDeviceContainer outputDevices2;     // A container of output NetDevices
    outputDevices2.Add(n3.Get(0));
    ...
    // multicast1.SetDefaultMulticastRoute(sender, senderIf);
